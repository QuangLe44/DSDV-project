...
function task4() {
    d3.csv(
        "https://raw.githubusercontent.com/QuangLe44/DSDV-project/main/csv/Central%20%26%20South%20America/Central%20%26%20South%20America%20-%20distribution%20losses.csv",
        function (error, data) {
            if (error) {
                console.log(error);
            } else {
                ...
                let xScale = d3.scaleTime()
                    .domain(d3.extent(netDistributionLosses, d => d.Date))
                    .range([p, w - p * 1.6]);
                
                let yScale = d3.scaleLinear()
                    .domain([0, d3.max(netDistributionLosses, d => d.Net)])
                    .range([h - p, p]);
                    ...
                    focusGroups.each(function (countryData) {
                      ...
                      countryText
                        .html(
                          ...
                          " DistributionLosses: " + dataForDate.Net
                        )
                ...
                svg
                    .append("text")
                    .text("Multiple Line Graph of Distribution Losses (South America)")
                    .attr("class", "axis-label")
                    .attr("text-anchor", "middle")
                    .attr("x", w / 2)
                    .attr("y", h - 750)
